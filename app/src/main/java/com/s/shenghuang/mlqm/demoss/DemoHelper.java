package com.s.shenghuang.mlqm.demoss;
...
import com.s.shenghuang.mlqm.demoss.db.DemoDBManager;
import com.s.shenghuang.mlqm.demoss.db.InviteMessgeDao;
import com.s.shenghuang.mlqm.demoss.db.UserDao;
import com.s.shenghuang.mlqm.demoss.domain.InviteMessage;
import com.s.shenghuang.mlqm.demoss.domain.RobotUser;
import com.s.shenghuang.mlqm.demoss.parse.UserProfileManager;
import com.s.shenghuang.mlqm.demoss.receiver.CallReceiver;
import com.s.shenghuang.mlqm.demoss.receiver.HeadsetReceiver;
            ...
            public String getDisplayedText(EMMessage message) {
                ...
                if(user != null){
                    if(EaseAtMessageHelper.get().isAtMeMsg(message)){
                        return String.format("", user.getNickname());
                    }
...
//                    intent = new Intent(appContext, VoiceCallActivity.class);
    ...
    public void goConference(String confId, String password, String extension) {
        ...
        try {
            ...
            inviter = jsonObj.optString(Constants.EXTRA_CONFERENCE_INVITER);
            groupId = jsonObj.optString(Constants.EXTRA_CONFERENCE_GROUP_ID);
...
//                break;
	...
	private EaseUser getUserInfo(String username){
        ...
        Map<String, EaseUser> contactList_db = demoModel.getContactList();
        Map<String, RobotUser> robotList = demoModel.getRobotList();
        if(user == null){
            if(contactList!=null&&contactList.size()>0){
                user=contactList.get(username);
            }else if(contactList_db!=null&&contactList_db.size()>0){
                user=contactList_db.get(username);
            }else if(robotList!=null&&robotList.size()>0){
    	...
    	messageListener = new EMMessageListener() {
			...
			public void onMessageReceived(List<EMMessage> messages) {
                ...
                for (EMMessage message : messages) {
                    ...
                    Log.e("环信头像","0"+userPic);
                    String uniqueId = message.getStringAttribute("uniqueId", "");
                    if(uniqueId!=null){
//                        AppSPUtils.setValueToPrefrences(application.instance(),Constants.SY_THER_UNIQUEID,uniqueId);
                    }
                    ...
                    // 存入db
//                    UserDao dao = new UserDao(application.instance());
...
//                    dao.saveContactList(users);
            ...
            public void onMessageRecalled(List<EMMessage> messages) {
                for (EMMessage msg : messages) {
                    ...
                    EMTextMessageBody txtBody = new EMTextMessageBody(String.format("This message has been recalled by %1$s", msg.getFrom()));
