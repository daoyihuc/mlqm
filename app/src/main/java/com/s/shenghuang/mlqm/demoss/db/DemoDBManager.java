package com.s.shenghuang.mlqm.demoss.db;
...
import com.s.shenghuang.mlqm.Applications;
...
public class DemoDBManager {
    ...
    private DemoDBManager(){
        dbHelper = DbOpenHelper.getInstance(Applications.instance().getApplicationContext());
    }
...
//            db.delete(UserDao.TABLE_NAME, null, null);
            for (EaseUser user : contactList) {
                ...
                Cursor query = db.query(UserDao.TABLE_NAME, null,
                        "username=?", new String[]{user.getUsername()}, null, null, null);
                if(query.moveToNext()){
                    String ord_nick = query.getString(query.getColumnIndex(UserDao.COLUMN_NAME_NICK));
                    String ord_avatar = query.getString(query.getColumnIndex(UserDao.COLUMN_NAME_AVATAR));
                    if(!ord_nick.equals(user.getUsername())&&!ord_avatar.equals( user.getAvatar())){
